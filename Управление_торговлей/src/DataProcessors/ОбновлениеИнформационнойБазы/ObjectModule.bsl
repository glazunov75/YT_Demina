...
Процедура ВыполнитьОбновлениеНаНовыйРелиз(НоваяВерсияИБ, мОбработчики, ДокументОписаниеОбновлений = "", МакетОписаниеОбновлений = "")
	
	ТекущаяВерсияИБ  = ОбновлениеИнформационнойБазы.ВерсияИБ(Метаданные.Имя);// Константы.НомерВерсииКонфигурации.Получить();
	НомерРелиза      = ПолучитьНомерРелиза(ТекущаяВерсияИБ);
	НовыйНомерРелиза = ПолучитьНомерРелиза(НоваяВерсияИБ);
	СтрокаВерсии     = "Версия" + СтрЗаменить(НовыйНомерРелиза, ".", "");
	СтрокаШапки      = "Шапка"  + СтрЗаменить(НовыйНомерРелиза, ".", "");
